    ...
    <div class="flex flex-col gap-3">
        ...
        <div v-for="plan in plans" class="flex gap-4 border-b-2 py-1 pb-3 w-full items-center">
            <form class="flex gap-4 " @submit.prevent="updatePlan(plan)">
                <label v-if="hasGoalField" class="flex gap-2 items-center whitespace-nowrap" for="penalty">
                    Вычет (%)
                    <input v-model="plan.data.penalty" class="input" name="penalty" type="number"
        ...
        <form v-if="isCurrentMonth && plansCount == 0" class="flex gap-4 border-b-2 py-1 pb-3"
            ...
            <label v-if="hasGoalField" class="flex gap-2 items-center whitespace-nowrap" for="penalty">
                Цель
                <input v-model="newPlan.data.penalty" class="input" name="penalty" type="number" />
            </label>
    ...
    methods: {
        updatePlan(plan) {
            ...
            if (this.hasGoalField) {
                data.penalty = plan.data.penalty;
            }
        ...
        createPlan() {
            let data = {};
            if (this.hasGoalField) {
                data.penalty = this.newPlan.data.penalty;
            }
            ...
            router.post(route('admin.projects-department.work-plan.store'), {
                ...
                onSuccess: () => {
                    this.newPlan.data.penalty = null;
