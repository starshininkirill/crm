        ...
        <form method="POST" class="max-w-md" action="{{ route('service.update', $service->id) }}">
            ...
            <div class="flex flex-col gap-4">
                ...
                <div>
                    ...
                    <select id="service_category_id" name="service_category_id" class="select">
                        ...
                        @foreach ($categories as $category)
                            <option
                                {{ isset($service->category) && $category->id == $service->category->id ? 'selected' : '' }}
                                value="{{ $category->id }}">
                                {{ $category->name }}
                            </option>
                        @endforeach
                ...
                @if (!empty($service->deal_template_ids))
                    ...
                    <x-form-input type="number" name="law_complex" placeholder="Юр. лицо комплекс"
                        label="Юр. лицо комплекс"
                ...
                @else
                    ...
                    <x-form-input type="number" name="law_complex" placeholder="Юр. лицо комплекс"
                        label="Юр. лицо комплекс" value="{{ old('law_complex') }}" />
                    <x-form-input type="number" name="physic_default" placeholder="Физ. лицо одна услуга"
