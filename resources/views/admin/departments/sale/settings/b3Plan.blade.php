<div class="flex flex-col gap-3">
    ...
    @php
        $plan = null;
        if ($plans->has($workPlanClass::B3_PLAN) && !$plans[$workPlanClass::B3_PLAN]->isEmpty()) {
            $plan = $plans[$workPlanClass::B3_PLAN][0];
        }
    @endphp
    <form class="flex gap-4 border-b-2 py-1 pb-3" method="POST"
        ...
        @if ($plan)
            @method('PUT')
        @endif
        <input type="hidden" name="type" value="{{ $workPlanClass::B3_PLAN }}">
